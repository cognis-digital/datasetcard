...
How **datasetcard** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `datasetcard` lives in the **Dataset / catalog** cluster.
...
| from | relation | to |
|---|---|---|
| `datasetcard` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis), [`awesome-drone-warfare-osint`](https://github.com/cognis-digital/awesome-drone-warfare-osint) |
| `datasetcard` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `datasetcard` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `datasetcard` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the dataset / catalog cluster.** `datasetcard` output feeds the next tool:
```bash
datasetcard ... --format json > out.json          # this tool's findings (see `datasetcard --help`)
...
datasetcard ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
datasetcard ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
datasetcard ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `datasetcard`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `datasetcard` slots
