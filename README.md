...
[![PyPI](https://img.shields.io/pypi/v/cognis-datasetcard.svg)](https://pypi.org/project/cognis-datasetcard/)
[![CI](https://github.com/cognis-digital/datasetcard/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/datasetcard/actions)
...
DATASETCARD exists for one job — auto dataset cards / datasheets with croissant + provenance — and does it without a SaaS bill or heavyweight setup.
...
```bash
datasetcard --version
datasetcard scan .                      # scan the current project
datasetcard scan . --format json
datasetcard scan . --fail-on high       # non-zero exit for CI gates
datasetcard mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **HF dataset cards**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`datasetcard` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
