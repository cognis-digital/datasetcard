...
- **rules/analyzers** apply the heuristics shipped in `datasetcard/core.py`.
- **score** ranks by severity.
- **MCP server** (`datasetcard mcp`) exposes `scan` for Cognis.Studio agents.
